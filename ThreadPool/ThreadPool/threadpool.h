...
public:
	...
	void start(int init_thread_size = std::thread::hardware_concurrency());		// 开启线程池，设定默认的初始线程数量为4，用户可以传入参数改变该值
	// std::thread::hardware_concurrency ：硬件并发 会返回运行该程序的计算机的处理器数量，Windows可以在 计算机管理-设备管理器-处理器 下查看，我的是12个处理器核心

	void setTaskSizeUpperLimit(int max_size);		// 设置任务队列数量上限

	void setThreadSizeUpperLimit(int max_size);		// cached模式下，设置线程队列数量上限
...
private:
	void threadFunc(int thread_id);		// 定义线程函数
...
private:
	...
	std::condition_variable pool_end;		// 线程池析构，等待线程全部回收
